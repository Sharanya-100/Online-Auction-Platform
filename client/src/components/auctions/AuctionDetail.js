...
const AuctionDetail = ({ showAlert }) => {
  ...
  useEffect(() => {
    const fetchAuctionData = async () => {
      try {
        ...
        setBidAmount(
          auctionRes.data.currentBid + auctionRes.data.minBidIncrement
        );
    ...
    if (socket && socket.socket) {
      ...
      const handleBidPlaced = (newBid) => {
        ...
        if (isAuthenticated && user && newBid.bidder._id !== user.id) {
          showAlert(
            `New bid placed: $${newBid.amount} by ${newBid.bidder.username}`,
            "info"
          );
  ...
  const handleBidSubmit = async (e) => {
    ...
    if (parseFloat(bidAmount) < auction.currentBid + auction.minBidIncrement) {
      showAlert(
        ...
        }. Minimum bid is $${auction.currentBid + auction.minBidIncrement}`,
        "warning"
      );
      return;
    }
  ...
  const isHighestBidder =
    ...
    auction.currentHighestBidder._id === user.id;
      ...
      <div className="row mb-4">
        <div className="col-md-8">
          ...
          <p className="text-muted">
            Listed by{" "}
            <span className="fw-bold">{auction.createdBy.username}</span> |
                      ...
                      isOwner ? (
                        ...
                        <form onSubmit={handleBidSubmit}>
                          ...
                          <div className="form-text">
                            You are about to bid {formatCurrency(bidAmount)}.
                  ...
                  <div className="d-flex">
                    ...
                    <div>
                      ...
                      <p className="mb-0">
                        {auction.currentHighestBidder ? (
                          <>
                            Final bid was {formatCurrency(auction.currentBid)}{" "}
                            by
                            <strong>
                              {" "}
                              {isHighestBidder
                  ...
                  <div className="d-flex">
                    ...
                    <div>
                      ...
                      <p className="mb-0">
                        ...
                        {formatDateTime(auction.startTime)}. Come back then to
                        place your bids!
                      </p>
                    </div>
                  </div>
          ...
          <div className="card shadow border-0 rounded-3 mb-4">
            ...
            <div className="card-body text-center">
              ...
              {auction.status === "active" && !isOwner && isAuthenticated && (
                <button
                  onClick={() =>
                    document.querySelector(".bid-form input").focus()
                  }
              ...
              {bids.length > 0 ? (
                <ul className="list-group list-group-flush">
                  {bids.map((bid) => (
                    <li
                      key={bid._id}
                      className="list-group-item px-3 py-3 bid-history-item"
                    >
